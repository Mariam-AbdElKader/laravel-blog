<x-admin.layout title="View Post #{{ $post->id }}">
    <div class="bg-gray-800 p-6 rounded-lg shadow-lg max-w-3xl mx-auto">
        ...
        <div class="mt-6 flex justify-end space-x-4">
            <a href="{{ route('admin.posts.index') }}" class="px-6 py-2 bg-gray-700 text-gray-200 rounded-md shadow hover:scale-105 transition">Back to Posts</a>
            <a href="{{ route('admin.posts.edit', $post) }}" class="px-6 py-2 bg-gradient-to-r from-purple-500 to-indigo-500 text-white rounded-md shadow hover:scale-105 transition">Edit Post</a>
            <form action="{{ route('admin.posts.destroy', $post) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this post?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-6 py-2 bg-red-600 text-white rounded-md shadow hover:scale-105 transition">Delete Post</button>
            </form>
        </div>
    </div>
</x-admin.layout>
